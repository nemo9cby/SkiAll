...
def is_excluded(path: Path | str) -> bool:
    ...
    pattern side via ``fnmatch``).  Files matching SAFE_PATTERNS (e.g.
    ``.env.example``) are always allowed through.
    """
    name = Path(path).name
    name_lower = name.lower()

    # Safe patterns always pass (templates, examples).
...
def generate_pre_commit_hook() -> str:
    ...
    safe_globs = " ".join(f'"{p}"' for p in SAFE_PATTERNS)

    hook = f"""\
#!/usr/bin/env bash
# SkiAll pre-commit hook -- blocks known secret files from being committed.
# Auto-generated by skiall.core.secrets; do not edit manually.
#
# Layer 1: Block files matching known secret filenames/patterns.
# Safe patterns (.example, .sample, .template) are always allowed.

set -euo pipefail

EXCLUDED_NAMES=({excluded_names})
EXCLUDED_GLOBS=({excluded_globs})
SAFE_GLOBS=({safe_globs})
...
while IFS= read -r -d '' file; do
    ...
    # Safe files always pass (e.g. .env.example)
    if is_safe "$base_lower"; then
        continue
    fi
    ...
    # Check glob pattern matches (case-insensitive)
